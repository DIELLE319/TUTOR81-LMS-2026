<?php
    ...
    if($op_type == 'new_course'){
        ...
        $res = $lesson_obj->editLesson($lesson_id,$title_lesson, $description_lesson, $duration_lesson, $percentage_lesson);
        	...
        	foreach ($learn_obj_order as $obj_id){
            $id = substr($obj_id, 4);
            if ($lesson_obj->isObjectInLesson($id, $lesson_id))	$lesson_obj->updatePositionLearningObjectInLesson($id, $lesson_id, $position);
            else $lesson_obj->addNewLearningObjectToLesson($id, $lesson_id, $position);
            $position++;
        	}
				}
