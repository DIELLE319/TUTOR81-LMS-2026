 ...
 * -----------------------------------------------------------------------------------------
 * 19-nov-2015
 * File: manage/learning_object.php
...
require_once BASE_LIBRARY_PATH . 'class_question_object.php';
...
if($op_type == "creat_new"){
	$title = filter_input(INPUT_POST, 'obj_title',  FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW | FILTER_FLAG_STRIP_HIGH);
	...
	$battute = $_POST['obj_battute'];

	$obj_lesson = 0;
	$obj_module = 0;

	$object_id = $learn_obj->createNewLearningObject($owner_user_id,$learning_object_type_id,$language_id,$percentage_correct_answer_to_pass,
			$description,$duration,$published_in_ecommerce,$obj_cat,$arg,$obj_lesson,$obj_module,$level_id, $title, $custom,$type_id,$battute);
	if ($object_id > 0){
		$title = trim($title);
		...
		$obj_om = $_POST['obj_om'];
		$temp_path = BASE_MEDIA_PATH . "user_store/$owner_user_id/learning_objects/temp";
		...
		if($learning_object_type_id == 1){
			...
			foreach ($obj_om as $om){
				...
				chmod($filename, 0777);
				rename($filename, BASE_MEDIA_PATH . "video/$video_path/$new_name.{$path_parts['extension']}");
			...
			$dest_path = BASE_MEDIA_PATH . "user_store/$owner_user_id/learning_objects/slide_test/$new_name.pdf";
			chmod($filename, 0755);
			rename($filename, $dest_path);
			$dir_images = BASE_MEDIA_PATH . "user_store/$owner_user_id/learning_objects/slide_test/images_of_$new_name/";
			mkdir($dir_images);
			...
			$filename = "$temp_path/$obj_om[0]";
			chmod($filename, 0755);
			rename($filename, BASE_MEDIA_PATH . "user_store/$owner_user_id/learning_objects/documents/$new_name.pdf");
			...
			rename($dirname, BASE_MEDIA_PATH . "user_store/$owner_user_id/learning_objects/web/$new_name");
	...
	$res = $learn_obj->disableByID($_POST['id']);
}elseif ($op_type == 'enable_it'){
	$res = $learn_obj->enableByID($_POST['id']);
	...
	$res = $learn_obj->editLearningObject($id, $language_id,
			$percentage_correct_answer_to_pass, $description, $duration, $battute, $title, $argument_id, $level_id, $obj_cat, $type_id, $custom);
	...
	$dirname = filter_input(INPUT_POST, 'dirname',  FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW | FILTER_FLAG_STRIP_HIGH);
	$user_id = filter_input(INPUT_POST, 'user_id', FILTER_SANITIZE_NUMBER_INT);
	$path = BASE_MEDIA_PATH . "template/web_objects/$dirname";
	if (is_dir($path) && $user_id > 0){
		$new_dir = uniqid("web_$dirname");
		$dest = BASE_MEDIA_PATH . "user_store/$user_id/learning_objects/temp/$new_dir";
		
		//recurseCopy($path, BASE_MEDIA_PATH . "user_store/$user_id/learning_objects/temp/$new_dir");
...
/*
 * Restituisce i corsi attivi che contengono l'oggetto
 */
}elseif($op_type === 'get_learning_project_by_om'){
    $omLearningProject = $learn_obj->getOmLearningProjects();
    $learning_projet_containing_om = array();
    if ($omLearningProject) 
        foreach ($omLearningProject as $learning_project){
            if (!key_exists($learning_project['learning_object_id'], $learning_projet_containing_om)) 
                    $learning_projet_containing_om[$learning_project['learning_object_id']] = array();
            array_push($learning_projet_containing_om[$learning_project['learning_object_id']], 
                array('learning_project_id' => $learning_project['learning_project_id'],
                      ...
                      'learning_project_published' => $learning_project['learning_project_published']));
    }
    $res = !empty($learning_projet_containing_om) ? json_encode($learning_projet_containing_om) : 0;
}
