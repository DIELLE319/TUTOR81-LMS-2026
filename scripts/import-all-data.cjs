...
async function main() {
  ...
  for (const c of clients) {
    if (existingIds.has(Number(c.id))) continue;

    try {
      await pg.query(`
        INSERT INTO companies (id, tutor_id, business_name, address, email, phone, city, is_active)
        VALUES ($1, $2, $3, $4, $5, $6, $7, true)
      `, [c.id, c.tutor_id, c.business_name, c.address, c.email, c.telephone, c.city]);
      newCompanies++;
  ...
  for (const c of corsisti) {
    if (seenIds.has(Number(c.id))) continue;
    seenIds.add(Number(c.id));
    
    if (!existingIds.has(Number(c.company_id))) continue;
  ...
  const totals = await pg.query(`
    SELECT 
      ...
      (SELECT COUNT(*) FROM tutors_purchases) as purchases
  `);
  console.log('\nTotali:');
  console.log(`Companies: ${totals.rows[0].companies}`);
  console.log(`Corsisti: ${totals.rows[0].students}`);
  console.log(`Vendite: ${totals.rows[0].purchases}`);
