...
$type = isset($area) ? $area : filter_input(INPUT_GET, 'type', FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW | FILTER_FLAG_STRIP_HIGH);

/* ***** VISTA ACQUISTI AZIENDA ***** */
if (!empty($company_id) || $type === 'company') {
    // acquisti company
...
/* ***** VISTA ACQUISTI ENTE FORMATIVO O SOCIO ***** */
} elseif (($type === 'tutor' || $type === 'member') && ($_SESSION['user']['role'] == 1000 || $_SESSION['user']['role'] == 1 || $_SESSION['user']['role'] == 32)) {
    ...
    /* ***** ACQUISTI SOCIO ***** */
    if ($type === 'member' && $_SESSION['user']['role'] == 32) {
...
} elseif ($type === 'admin' && $_SESSION['user']['role'] == 1000) {
    // acquisti superadmin
