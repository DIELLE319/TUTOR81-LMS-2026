var RefreshTable = function(o, j) {
    ...
    var option = $('.employees-table').data('option');
    var l = $("#" + option + "-table").DataTable();
    l.clear().draw().on('draw', function(){$.isLoading("hide");});
    $.get("lib/employee_search.php?option=" + option + "&term=&learningprojectid=" + m + "&companyid=" + h + "&unita=" + o + "&reparto=" + j, function(q) {
        var p = $.parseJSON(q);
        l.rows.add(p);
        l.draw();
    });
...
$(document).ready(function() {
    ...
    $(".sell").on("click", function() {
        var n = $(this).parentsUntil(".modal-body").find(".wizard-steps").find(".active");
        var m = $("#ecom-licences");
        var j = m.find(".learningProjectID").val();
        console.log("CourseID: " + j);
        $.isLoading({
            text: "Attendere prego ..."
        });
        var p = $("#ecom-orders tr").find('.rowLearningProjectID[value="' + j + '"]').parent("td").parent("tr");
        console.log(p);
        var i = $("#userID").val();
        var w = $("#tutorCompanyID").val();
        var s = $("#tutorEmail").val();
        b = $("#clienteCompanyID").val();
        console.log("customer_company_id: " + b);
        var q = 0;
        var o = p.find(".productId").text();
        console.log("learning_project_id: " + o);
        var x = 0;
        var y;
        var t;
        var z;
        ...
        var l = "backend";
        console.log("Payment type: " + l);
        var h = $(this);
        var k;
        if (n.attr("id") != "nuovoUtenteTab") {
            console.log("INVIA LICENZE NUOVO UTENTE");
            k = $(".employee-table-body").children("tr").filter(function(A) {
                var B = $(this).find(".select-checkbox");
                if (B.is(":checked")) {
                    return A
                }
            });
            console.log("Row licences list:");
            console.log(k);
            y = k.length;
            console.log("Numero di licenze generato: " + y);
            t = p.find(".productPrice").text();
            console.log("Prezzo di vendita: " + t);
            z = r() + v();
            console.log("Totale della vendita: " + z);
            console.log("Inizio generazione della nuova vendita...");
            $.post("/ecommerce/license.php", {
                op_type: "new_ecommercetutorbackend_purchase",
                tutorid: i,
                customercompany_id: b,
                usercompany_ref: q,
                learningproject_id: o,
                amount: y,
                costcentre_id: x,
                price: z,
                payment: l,
                ...
                console.log("Completata generazione della nuova vendita.");
                var A = B;
                console.log("Codice della nuova vendita: " + B);
                console.log("Inizio generazione ed invio dei codici corso agli utenti...");
                if (A > 0) {
                    k.each(function() {
                        console.log("Riga della tabella:");
                        console.log($(this));
                        var F = $(this);
                        var K = "";
                        var H = new Date;
                        console.log(H);
                        var N = new Date(new Date().setFullYear(new Date().getFullYear() + 1));
                        console.log(N);
                        var G = 15;
                        var C = "";
                        var O = "";
                        ...
                        var D = "";
                        var J = "";
                        var I = F.find(".licenceIDUser").text();
                        console.log("UsesID assegnatario: " + I);
                        $.post("/ecommerce/license.php", {
                            ...
                            user_companyid: b,
                            learningproject_id: o,
                            learningproject_data_inizio: H,
                            learningproject_data_fine: N,
                            user_alert_days: G,
                            user_nome: C,
                            user_cognome: O,
                            user_cod_fisc: M,
                            user_data_assunzione: L,
                            user_unita: E,
                            user_reparto: D,
                            user_tipo_utente: J,
                            user_email: K,
                            ...
                            console.log(P);
                            console.log("Corso venduto....")
                        })
                    });
                    alert("Ti confermiamo l'invio di numero " + y + " licenza/e per il seguente corso: " + f.text() + ".");
                    location.reload(true)
                } else {
                    $.isLoading("hide");
                    alert("Non è stato possibile completare la vendita, si prega di contattare il servizio di assistenza della piattaforma.");
                    location.reload(true)
                }
            })
        } else {
            console.log("INVIA LICENZE NUOVO UTENTE");
            k = $("#sendLicenceForm").find(".sendLiceneceRowsContainer");
            console.log("Row licences list:");
            console.log(k);
            var u = false;
            k.find(".inviaRows").each(function() {
                if ($(this).find(".email").val() == "" && $(this).find(".email").css("border", "1px solid red")) {
                    $(this).find(".email").css("border", "1px solid red");
                    u = true
                }
                var C = $(this).find(".cognome");
                var A = $(this).find(".nome");
                var B = $(this).find(".cod_fisc");
                if (C.val() != "" || A.val() != "" || B.val() != "") {
                    if (C.val() == "") {
            ...
            if (u) {
                alert("Completate tutti i campi obbligatori segnalati in rosso.");
                $.isLoading("hide");
                return false
            }
            y = m.find(".productQuantita").val();
            console.log("Numero di licenze generato: " + y);
            t = p.find(".productPrice").text();
            console.log("Prezzo di vendita: " + t);
            z = r() + v();
            ...
            $.post("/ecommerce/license.php", {
                ...
                if (A > 0) {
                    k.find(".inviaRows").each(function() {
                        console.log($(this));
                        var F = $(this);
                        var K = F.find(".email").val();
                        console.log(K);
                        var H = F.find(".data_inizio").val();
                        var N = F.find(".data_fine").val();
                        var G = F.find(".alert_days").val();
                        var C = F.find(".nome").val();
                        var O = F.find(".cognome").val();
                        var M = F.find(".cod_fisc").val();
                        var L = F.find(".data_assunzione").val();
                        var E = F.find(".unita").val();
                        var D = F.find(".reparto").val();
                        var J = F.find(".tipo_utente").val();
                        var I = F.find(".licenceIDUser").val();
                        $.post("/ecommerce/license.php", {
                            op_type: "new_ecommercetutorbackend_assignment",
                            tutorid: i,
                            purchase_id: A,
    ...
    $("body").on("hidden.bs.modal", ".modal", function() {
        ...
        if (h.val() == 1) {
            ...
            $(".inviaRows .form-group").addClass("verticalRowWidth");
            $(".inviaRows div:first-child").removeClass("email-field");
            $(".inviaRows div:nth-child(2)").removeClass("startEndDate-field");
            $(".inviaRows div:nth-child(3)").removeClass("startEndDate-field");
            $(".inviaRows div:nth-child(4)").removeClass("alert-field");
    ...
    $("#clienteCompanyID").on("change", function() {
        ...
        $('#clienteCompanyID option:selected').attr('selected', true); 
        $("select[name=department]").addClass("hidden");
        var utenteEsistenteTab = $("#utenteEsistenteTab").attr("class");
        //console.log("Change...");
        //console.log(utenteEsistenteTab);
        if (utenteEsistenteTab == "active") {
            //console.log("Active...");
            var clienteCompanyID = $("#clienteCompanyID").val();
            //console.log("Company ids: " + clienteCompanyID);
            var employees_table = $(".employees-table").DataTable();
            $(".product_unit > div.dep_controls").empty();
            $.post("manage/department.php", {
                op_type: "get_pu",
                pu_id: clienteCompanyID
            }, function(data) {
                var column_value = '<option value="0">Seleziona unità</option>';
                data = $.parseJSON(data);
                if (data == 0) {
                    $(".product_unit > div.pu_controls select").empty().append(column_value);
                } else {
                    $.each(data, function(index, value) {
                        column_value += '<option value="' + value.id_pu + '">' + value.short_desc_pu + "</option>";
                        if (index == Object.keys(data).length - 1) {
                            $(".product_unit > div.pu_controls select").empty().append(column_value);
                        }
                    })
                }
                var pu_length = $(".pu_select option").length;
                //console.log("LengthAfterPost " + pu_length);
                if (pu_length > 1) {
                    $(".pu_select").removeClass("hidden");
                    $(".employees-table").DataTable().clear().draw();
                } else {
